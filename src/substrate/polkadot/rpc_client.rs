...
use crate::rpc_client::{NotificationStream, RpcClients};
use crate::substrate::handlers::{
    ChainAllHeadDispatcher, ChainFinalizedHeadDispatcher, ChainNewHeadDispatcher,
    StateRuntimeVersionDispatcher, StateStorageDispatcher,
};
use crate::substrate::rpc_client::{SubscriptionDispatcher, SubstrateClient};
use crate::{
...
pub struct SubscribedStream {
    ...
    handles: HashMap<&'static str, Box<dyn Fn(WsConnections)>>,
}

pub async fn register_subscriptions(client: &mut SubstrateClient, chain: Chain) {
    client.register_handler(Box::new(StateStorageDispatcher::new(chain)));
    client.register_handler(Box::new(StateRuntimeVersionDispatcher::new(chain)));
    client.register_handler(Box::new(ChainNewHeadDispatcher::new(chain)));
    client.register_handler(Box::new(ChainFinalizedHeadDispatcher::new(chain)));
    client.register_handler(Box::new(ChainAllHeadDispatcher::new(chain)));
}
